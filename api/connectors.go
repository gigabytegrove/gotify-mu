...
func (a *ConnectorAPI) CreateSMTPRoute(ctx *gin.Context){var p smtpRouteParams;if err:=ctx.ShouldBindJSON(&p);err!=nil{return};if !a.channel(ctx,p.ApplicationID){return};item:=&model.SMTPRoute{Name:p.Name,ApplicationID:p.ApplicationID,Recipient:strings.ToLower(strings.TrimSpace(p.Recipient)),AllowedCIDRs:p.AllowedCIDRs,Username:p.Username,Password:p.Password,Enabled:p.Enabled};if !successOrAbort(ctx,500,a.DB.SaveSMTPRoute(item)){return};ctx.JSON(201,smtpView(item))}
func (a *ConnectorAPI) UpdateSMTPRoute(ctx *gin.Context){withID(ctx,"id",func(id uint){item,err:=a.DB.GetSMTPRouteByID(id);if !successOrAbort(ctx,500,err){return};if item==nil{ctx.AbortWithStatus(404);return};var p smtpRouteParams;if err:=ctx.ShouldBindJSON(&p);err!=nil{return};if !a.channel(ctx,p.ApplicationID){return};item.Name=p.Name;item.ApplicationID=p.ApplicationID;item.Recipient=strings.ToLower(strings.TrimSpace(p.Recipient));item.AllowedCIDRs=p.AllowedCIDRs;item.Username=p.Username;if p.Password!=""{item.Password=p.Password};item.Enabled=p.Enabled;if !successOrAbort(ctx,500,a.DB.SaveSMTPRoute(item)){return};ctx.JSON(200,smtpView(item))})}
...
type rssParams struct { Name string `json:"name" binding:"required"`; ApplicationID uint `json:"applicationId" binding:"required"`; URL string `json:"url" binding:"required"`; IntervalMinutes int `json:"intervalMinutes"`; Enabled bool `json:"enabled"` }
...
func (a *ConnectorAPI) CreateRSS(ctx *gin.Context){var p rssParams;if err:=ctx.ShouldBindJSON(&p);err!=nil{return};if !a.channel(ctx,p.ApplicationID){return};if err:=connectors.ValidateConnectorURL(p.URL);err!=nil{ctx.AbortWithError(400,err);return};if p.IntervalMinutes<1{p.IntervalMinutes=15};item:=&model.RSSMonitor{Name:p.Name,ApplicationID:p.ApplicationID,URL:p.URL,IntervalMinutes:p.IntervalMinutes,Enabled:p.Enabled};if !successOrAbort(ctx,500,a.DB.SaveRSSMonitor(item)){return};ctx.JSON(201,item)}
func (a *ConnectorAPI) UpdateRSS(ctx *gin.Context){withID(ctx,"id",func(id uint){item,err:=a.DB.GetRSSMonitorByID(id);if !successOrAbort(ctx,500,err){return};if item==nil{ctx.AbortWithStatus(404);return};var p rssParams;if err:=ctx.ShouldBindJSON(&p);err!=nil{return};if !a.channel(ctx,p.ApplicationID){return};if err:=connectors.ValidateConnectorURL(p.URL);err!=nil{ctx.AbortWithError(400,err);return};if p.IntervalMinutes<1{p.IntervalMinutes=15};item.Name=p.Name;item.ApplicationID=p.ApplicationID;item.URL=p.URL;item.IntervalMinutes=p.IntervalMinutes;item.Enabled=p.Enabled;if !successOrAbort(ctx,500,a.DB.SaveRSSMonitor(item)){return};ctx.JSON(200,item)})}
func (a *ConnectorAPI) DeleteRSS(ctx *gin.Context){withID(ctx,"id",func(id uint){successOrAbort(ctx,500,a.DB.DeleteRSSMonitor(id))})}

type syslogParams struct { Name string `json:"name" binding:"required"`; ApplicationID uint `json:"applicationId" binding:"required"`; Facility int `json:"facility"`; MaxSeverity int `json:"maxSeverity"`; AllowedCIDRs string `json:"allowedCidrs"`; Enabled bool `json:"enabled"` }
...
func (a *ConnectorAPI) CreateSyslog(ctx *gin.Context){var p syslogParams;if err:=ctx.ShouldBindJSON(&p);err!=nil{return};if !a.channel(ctx,p.ApplicationID){return};if p.Facility < -1 || p.Facility>23 || p.MaxSeverity<0 || p.MaxSeverity>7{ctx.AbortWithError(400,errors.New("invalid syslog facility or severity"));return};item:=&model.SyslogRoute{Name:p.Name,ApplicationID:p.ApplicationID,Facility:p.Facility,MaxSeverity:p.MaxSeverity,AllowedCIDRs:p.AllowedCIDRs,Enabled:p.Enabled};if !successOrAbort(ctx,500,a.DB.SaveSyslogRoute(item)){return};ctx.JSON(201,item)}
func (a *ConnectorAPI) UpdateSyslog(ctx *gin.Context){withID(ctx,"id",func(id uint){item,err:=a.DB.GetSyslogRouteByID(id);if !successOrAbort(ctx,500,err){return};if item==nil{ctx.AbortWithStatus(404);return};var p syslogParams;if err:=ctx.ShouldBindJSON(&p);err!=nil{return};if !a.channel(ctx,p.ApplicationID){return};if p.Facility < -1 || p.Facility>23 || p.MaxSeverity<0 || p.MaxSeverity>7{ctx.AbortWithError(400,errors.New("invalid syslog facility or severity"));return};item.Name=p.Name;item.ApplicationID=p.ApplicationID;item.Facility=p.Facility;item.MaxSeverity=p.MaxSeverity;item.AllowedCIDRs=p.AllowedCIDRs;item.Enabled=p.Enabled;if !successOrAbort(ctx,500,a.DB.SaveSyslogRoute(item)){return};ctx.JSON(200,item)})}
...
type calendarParams struct { Name string `json:"name" binding:"required"`; ApplicationID uint `json:"applicationId" binding:"required"`; URL string `json:"url" binding:"required"`; IntervalMinutes int `json:"intervalMinutes"`; NotifyBeforeMinutes int `json:"notifyBeforeMinutes"`; Enabled bool `json:"enabled"` }
