...
func (a *UserAPI) CreateUser(ctx *gin.Context) {
	...
	if err := ctx.Bind(&user); err == nil {
		if err := password.ValidateNewPasswordWithMin(user.Pass, a.PasswordMinLength); err != nil {
...
func (a *UserAPI) ChangePassword(ctx *gin.Context) {
	...
	if err := ctx.Bind(&pw); err == nil {
		if err := password.ValidateNewPasswordWithMin(pw.Pass, a.PasswordMinLength); err != nil {
			ctx.AbortWithError(http.StatusBadRequest, err)
			return
		}
		user, err := a.DB.GetUserByID(auth.GetUserID(ctx))
	...
	withID(ctx, "id", func(id uint) {
		...
		if err := ctx.Bind(&updatedUser); err == nil {
			...
			if dbUser != nil {
				...
				if updatedUser.Pass != "" {
					if err := password.ValidateNewPasswordWithMin(updatedUser.Pass, a.PasswordMinLength); err != nil {
						ctx.AbortWithError(http.StatusBadRequest, err)
						return
					}
